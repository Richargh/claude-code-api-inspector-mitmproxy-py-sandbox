= Claude Evaluation with Mitmproxy

This project exposes precisely what Claude Code requests and receives in response from the LLM Web Api.
Useful if you want to know what the agent is doing and what the context is that leads to it's decisions.
...
1. Run tests with `PYTHONPATH=src python -m unittest discover -s test -p "test_*.py"`.
